...
from src.scripts.utils import download_source_file_from_s3

# # dimensions of text-ada-embedding-002
d = 1536

# load all chains
from src.scripts.llm_chains import chains
...
def get_summarize_chain(task, input_sentence, prompt_text, source):
    flag_single_prompt, permissible_chunk_size = check_single_prompt(
        input_sentence, task, prompt_text
    )
    if flag_single_prompt == 1 and source == "video":
        llm_chain = chains.llm_chain_stuff_transcript_summary
    elif flag_single_prompt == 1 and source == "comments":
        llm_chain = chains.llm_chain_stuff_comments_summary
    elif source == "video":
        llm_chain = chains.llm_chain_map_reduce_transcript_summary
    elif source == "comments":
        llm_chain = chains.llm_chain_map_reduce_comments_summary
...
def get_intent(question):
    llm_chain = chains.llm_chain_intent
...
def get_summary(text_json, source, video_id, force_train=False):
    try:
        ...
        llm_chain, flag_single_prompt, permissible_chunk_size = get_summarize_chain(
            "Summarize", text, prompt_text, source
        )
        docs = get_documents(
            input_sentence=text,
...
def get_answer_from_summary(question, source, video_id):
    ...
    if source == "video":
        llm_chain = chains.llm_chain_transcript_summary_qa
    else:
        llm_chain = chains.llm_chain_comments_summary_qa
